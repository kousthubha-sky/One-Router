...
function RazorpaySetupContent() {
  ...
  const save = async () => {
    setError("");
    setSuccess("");
    const prefix = env === "live" ? "rzp_live_" : "rzp_test_";
    if (!keyId.startsWith(prefix)) {
      setError(`Key must start with ${prefix}`);
      return;
    }
    setSaving(true);
    try {
      await apiClient("/api/razorpay/credentials", {
        method: "POST",
        body: JSON.stringify({
          environment: env,
          key_id: keyId,
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    placeholder={env === "live" ? "rzp_live_..." : "rzp_test_..."}
                ...
                <div>
                  <label className="text-xs text-zinc-500 mb-1.5 block">Key Secret</label>
                  <input
                    type={showSecrets ? "text" : "password"}
                    value={keySecret}
                ...
                <div>
                  <label className="text-xs text-zinc-500 mb-1.5 block">
                    Webhook Secret <span className="text-zinc-700">(optional)</span>
                  </label>
                  <input
                    type={showSecrets ? "text" : "password"}
                    value={webhookSecret}
